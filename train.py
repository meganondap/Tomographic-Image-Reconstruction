...
Train FNO on tomography pairs (X_true -> Y_tilde), using MATLAB.mat files.

Each.mat file (sample_XXXXX.mat) contains:
    - X_true_mat   : [H,W] double
    - Y_mat        : [H,W] double
    - Y_tilde_mat  : [H,W] double
    - global_min   : scalar
    - global_max   : scalar

In this script:
    - We load x = X_true_mat
    - We load y = Y_tilde_mat
    - We DO NOT apply any additional global scaling in Python.
    - The model learns to predict Y_tilde directly.
"""
...
PHASE_ID = "phase_mat_ytilde_only"
...
# ---- Data / splitting ----
MAX_SAMPLES    = 10      # cap dataset at N pairs
TRAIN_FRACTION = 0.7
TRAIN_BATCH_SIZE = 2
...
H1_WEIGHT  = 0.1   # weight for H1 loss
...
class TomographyMatDatasetYtilde(Dataset):
    """
    Dataset for (X_true, Y_tilde) pairs stored in MATLAB.mat files.

    Each original.mat file is expected to have:
        - X_true_mat   : [H,W] double
        - Y_tilde_mat  : [H,W] double
        - Y_mat        : [H,W] double (not used here)
        - global_min   : scalar
        - global_max   : scalar
    ...
    def __getitem__(self, idx):
        ...
        if "Y_tilde_mat" not in mat:
            raise KeyError(f"'Y_tilde_mat' not found in {mat_path}")

        X_true = mat["X_true_mat"].astype(np.float32)   # [H,W]
        Y_tilde = mat["Y_tilde_mat"].astype(np.float32) # [H,W]

        x = torch.from_numpy(X_true).unsqueeze(0)   # [1,H,W]
        y = torch.from_numpy(Y_tilde).unsqueeze(0)  # [1,H,W]
...
base_dataset = TomographyMatDatasetYtilde(mat_dir=MAT_DIR)
...
def combined_train_loss(y_pred, y, **kwargs):
    """
    Weighted sum of L2 and H1 losses.

    y_pred : model output (Y_tilde_pred)
    y      : ground truth (Y_tilde_mat)
    """
    l2 = l2loss(y_pred, y)
    h1 = h1loss(y_pred, y)
